...
@EnabledIfEnvironmentVariable(
        named = "ASSESSMENT_RULE_PERCENTILE_SIMULATION_ENABLED",
        matches = "true"
)
class RuleApplicationPercentileCsvExecutionTest {
    ...
    @Test
    @DisplayName("규칙별 적용 횟수 백분위를 조건별 10,000명에게서 계산한다.")
    void exportRuleApplicationPercentileCsv() {
        int simulationCount = getSimulationCount();
        long randomSeed = getRandomSeed();
        Path outputDirectory = Path.of(
                "build",
                "assessment-simulation",
                "rule-percentile-seed-" + randomSeed
        );
    ...
    private int getSimulationCount() {
        String simulationCount = System.getenv("ASSESSMENT_RULE_PERCENTILE_SIMULATION_COUNT");
    ...
    private long getRandomSeed() {
        String randomSeed = System.getenv("ASSESSMENT_RULE_PERCENTILE_SIMULATION_SEED");
